...
%typemap(in)        std::map const &INPUT (std::vector<std::function<void()>> _global_freeargs) {
  ASSERT_STRING_MAP;
  if ($input.IsObject()) {
    $1 = new $*ltype;
    ...
    for (size_t i = 0; i < keys.Length(); i++) {
      ...
      _global_freeargs.emplace_back([c_val, c_key]() {
...
%typemap(in)        std::map INPUT (std::vector<std::function<void()>> _global_freeargs) {
  ASSERT_STRING_MAP;
  if ($input.IsObject()) {
    Napi::Object obj = $input.As<Napi::Object>();
    Napi::Array keys = obj.GetPropertyNames();
